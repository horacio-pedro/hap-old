...
export class EmployeeJobPostService {
	...
	public async updateVisibility(
		...
		return await this.hapAIService.updateVisibility(input);
	}
	...
	public async updateApplied(
		...
		return await this.hapAIService.updateApplied(input);
	}

	/**
	 * Find all available Jobs matched to hap Employees
	 * @param data
	 */
	public async findAll(
		...
		if (env.hapAIGraphQLEndpoint) {
			const result = await this.hapAIService.getEmployeesJobPosts(data);

			if (result === null) {
				if (env.production) {
					// OK, so for some reason connection go hap AI failed, we can't get jobs ...
					jobs = {
						items: [],
						total: 0
					};
