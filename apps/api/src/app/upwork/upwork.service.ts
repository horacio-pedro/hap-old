...
export class UpworkService {
	...
	// engagement has access to contract ID, this is a project in hap
	...
	async syncContractsRelatedData({
		...
		if (!employeeId) {
			const employee = await this._getUpworkHapEmployee(
				providerRefernceId,
				integrationId,
				organizationId,
				config
		...
		try {
			...
			const integratedTimeSlotsMinutes = await Promise.all(
				minutes.map(async (minute) => {
					...
					const hapTimeSlotMinute = await this.commandBus.execute(
	...
	private async _getUpworkHapEmployee(
		providerRefernceId,
		integrationId,
		organizationId,
		config: IUpworkApiConfig
		...
		return await Promise.all(
			rows
				...
				.map(async (row) => {
					...
					const hapExpense = await this.commandBus.execute(
	...
	private async _syncIncome(
		...
		for await (const row of rows) {
			...
			if (income) {
				...
				const hapIncome = await this.commandBus.execute(
	...
	async getReportListByIntegration(
		...
		const hapIds = _.pluck(items, 'hapId');
		const {
			dateRange: { start, end }
		} = filter;
				...
				.map(async (row) => {
					...
					for await (const item of items) {
						...
						if (
							...
							const hapOffer = await this.commandBus.execute(
