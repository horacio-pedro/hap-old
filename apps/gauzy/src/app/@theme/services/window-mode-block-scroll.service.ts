...
export class WindowModeBlockScrollService implements OnDestroy {
	...
	private updateContentSizeAndPosition() {
		...
		this.content.style.left = coerceCssPixelValue(
			-this.previousScrollPosition.left + left
		);
		this.content.style.top = coerceCssPixelValue(
			-this.previousScrollPosition.top + top
		);
		this.layout
			.getDimensions()
			.pipe(
				map(({ clientWidth }: NbLayoutDimensions) =>
					coerceCssPixelValue(clientWidth)
				),
				take(1)
			)
			.subscribe(
				(clientWidth: string) =>
					(this.content.style.width = clientWidth)
			);
	}
